...
int main()
{
	...
	glm::vec3 cubePositions[] = {	// used to render the cube at several locations
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// bind VAO describing the cube
		glGenVertexArrays(1, &VAO);
		for (GLuint i = 0; i < 10; ++i)
		{
			// creating the model matrix for each cube at different location
			glm::mat4 model = glm::mat4(1.0f);
			model = glm::translate(model, cubePositions[i]);
			// specifying unique rotation for each cube
			GLfloat angle = 20.0f * i;
			model = glm::rotate(model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));

			// assign the model matrix to the uniform representing the model matrix in the vertex shader
